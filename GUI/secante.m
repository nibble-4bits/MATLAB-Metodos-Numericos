...
% Last Modified by GUIDE v2.5 04-May-2018 01:00:30
...
try
    ...
    errordlg('Valor x0 no válido', 'Error');
end

try
    x1 = eval(get(handles.txtX1, 'String'));
catch
    errordlg('Valor x1 no válido', 'Error');
end
...
if isfinite(funcion) && isfinite(x0) && isreal(x0) && isfinite(porcError) && isreal(porcError)
    ...
    pause(0.1);
    [xn, xn_1, error, numIter] = metodoSecante(funcion, x0, x1, porcError);
